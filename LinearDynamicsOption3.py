...
class FOM_simulation(object):
    ...
    def GetInitialDisplacement(self, applied_force):
        ...
        Displacement_old_1 = np.zeros((np.shape(self.T)[1]))
        Displacement_old_1[1:] = np.squeeze(np.linalg.solve(self.K1[1:,1:], applied_force_vector[1:]))
        Displacement_old[1:]= self.T[1:,1:] @ Displacement_old_1[1:]

        return Displacement_old, Displacement_old_1
    ...
    def Solve(self):
        ...
        applied_force = 1
        Displacement_old, Displacement_old_1 = self.GetInitialDisplacement(applied_force) #solve static problem to determine original deformation state

        Displacement_new = Displacement_old.copy()
        Displacement_new_1 = Displacement_old_1.copy()
        ...
        self.SnapshotsMatrixDisplacements_1 = np.zeros((np.shape(Displacement_old_1)[0],self.number_of_time_steps))
        self.SnapshotsMatrixDisplacements_1[:,0] = Displacement_old_1
        ...
        for i in range(1,self.number_of_time_steps):
            ...
            Displacement_new_1[1:] = np.linalg.solve(K_hat[1:,1:], F_hat[1:])
            Displacement_new[1:] = self.T[1:,1:] @ Displacement_new_1[1:]
            ...
            self.SnapshotsMatrixDisplacements_1[:,i] = Displacement_new_1
            ...
            Displacement_old_1 = Displacement_new_1.copy()
...
class ROM_simulation(FOM_simulation):
    ...
    def ComputeSystemMatrix(self):
        ...
        self.matrixD = fom_simulation.T @ self.basis
    ...
    def Solve(self):
        ...
        Velocity_old_rom = np.zeros((self.domain.number_of_nodes))
        Acceleration_old_rom = np.zeros((self.domain.number_of_nodes))

        self.SnapshotsMatrixDisplacements_rom = np.zeros((self.domain.number_of_nodes,self.number_of_time_steps))
        self.SnapshotsMatrixDisplacements_rom[:,0] = Displacement_old_rom
        self.SnapshotsMatrixVelocities_rom = np.zeros((self.domain.number_of_nodes,self.number_of_time_steps))
        self.SnapshotsMatrixVelocities_rom[:,0] = Velocity_old_rom
        self.SnapshotsMatrixAccelerations_rom = np.zeros((self.domain.number_of_nodes,self.number_of_time_steps))
...
basis = compute_basis(fom_simulation.SnapshotsMatrixDisplacements_1, 5e-8)
